...
impl<T: IntoResponse> IntoResponse for Xml<T> {
    fn into_response(self) -> axum::response::Response {
        ([(header::CONTENT_TYPE, "application/rss+xml")], self.0).into_response()
    }
}

async fn world(
    Path(name): Path<String>,
    client: Extension<ReqwestClient>,
    client_id: Extension<ClientId>,
    client_secret: Extension<ClientSecret>,
) -> Result<String, TwitchRssError> {
    let token = get_token(&client.0, client_id.0.clone(), client_secret.0.clone()).await?;

    let helix_client = HelixClient::with_client(client.0);
...
async fn channel(
    ...
    let token = get_token(&client.0, client_id.0, client_secret.0).await?;
